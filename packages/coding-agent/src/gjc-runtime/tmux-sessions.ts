...
function tryKillSession(sessionName: string, env: NodeJS.ProcessEnv): void {
	try {
		runTmux(["kill-session", "-t", normalizeExactTmuxTarget(sessionName, env, "session")], env);
	} catch {
...
export function createGjcTmuxSession(env: NodeJS.ProcessEnv = process.env): GjcTmuxSessionStatus {
	...
	function publishOwnerGenerationSync(
		stateDir: string,
		sessionId: string,
		generation: string,
		stateFile: string,
		...
		const lifecycle = lifecyclePaths(stateDir, sessionId, generation);
		fsSync.mkdirSync(lifecycle.root, { recursive: true });
		const temporary = `${lifecycle.generationFile}.${crypto.randomUUID()}.tmp`;
		...
		try {
			...
			fsSync.renameSync(temporary, lifecycle.generationFile);
			const directory = fsSync.openSync(lifecycle.root, "r");
			try {
	...
	publishOwnerGenerationSync(stateDir, sessionId, generation, stateFile, tmuxCommand);
	...
	const outcome = (createOwnerIsolationTestDependencies?.execute ?? executeTmuxOwnerIsolationPlanSync)(ownerPlan, {
		...
		spawn: (argv, stdinLine) => {
			const result = stdinLine
				? Bun.spawnSync(argv, { stdout: "pipe", stderr: "pipe", stdin: Buffer.from(stdinLine), env })
				: Bun.spawnSync(argv, { stdout: "pipe", stderr: "pipe", env });
			return { exitCode: result.exitCode, stdout: result.stdout.toString() };
		},
		probeServer: ownerProbe.probeServer,
		isCurrentGeneration: () => isCurrentGenerationSync(stateDir, sessionId, generation),
	});
	if (!outcome.ok) throw new Error(`gjc_tmux_owner_isolation_${outcome.code}:${outcome.diagnostic}`);
	const isPsmux = resolveGjcTmuxBinary({ env }).isPsmux;
	if (isPsmux) throw new Error("gjc_tmux_owner_isolation_native_session_identity_unavailable");
	...
	try {
		const target = nativeSessionId ? `${nativeSessionId}:` : buildGjcTmuxExactOptionTarget(sessionName, { env });
		...
		try {
			const server = requireSafeTmuxServerForMutation(tmuxCommand, env);
			if (
				server.pid === outcome.server_pid &&
				server.startTime === outcome.server_start_time &&
				readNativeTmuxSessionId(nativeSessionId, env) === nativeSessionId
			)
				tryKillSession(nativeSessionId, env);
		} catch {}
		throw error;
	}
	if (nativeSessionId) {
...
export function proveGjcTmuxSessionMutationTarget(
	...
	if (finalServer.pid !== firstServer.pid || finalServer.startTime !== firstServer.startTime)
		throw new Error(`gjc_tmux_owner_changed:${sessionName}`);
	return { nativeSessionId, serverPid: finalServer.pid, serverStartTime: finalServer.startTime };
}

function cleanupExactTmuxSession(sessionName: string, env: NodeJS.ProcessEnv): void {
	try {
		runTmux(["kill-session", "-t", normalizeExactTmuxTarget(sessionName, env, "session")], env);
	} catch (error) {
		const message = error instanceof Error ? error.message : String(error);
		if (/no server running|can't find session|no such session/i.test(message)) return;
		throw error;
	}
}
...
export function removeGjcTmuxSession(sessionName: string, env: NodeJS.ProcessEnv = process.env): GjcTmuxSessionStatus {
	...
	if (finalServer.pid !== firstServer.pid || finalServer.startTime !== firstServer.startTime)
		throw new Error(`gjc_tmux_owner_changed:${sessionName}`);
	runTmux(["kill-session", "-t", normalizeExactTmuxTarget(nativeSessionId, env, "session")], env);
	return session;
}
...
function isCurrentGenerationSync(stateDir: string, sessionId: string, generation: string): boolean {
	try {
		const current: unknown = JSON.parse(
			fsSync.readFileSync(path.join(stateDir, sessionId, "owner-lifecycle", "generation.json"), "utf8"),
		);
		return (
...
async function resolveExactOwner(
	...
	return { sessionId, stateDir, socketKey: ownerServerKey, generation, pid, startTime };
}
...
async function waitForExpectedVerdict(
	...
	const verdictFile = lifecyclePaths(identity.stateDir, identity.sessionId, identity.generation).verdictFile;
	const verdictAliasFile = path.join(identity.stateDir, "verdict.json");
	while (now().getTime() <= deadline) {
		try {
			...
			if (
				...
				typeof alias === "object" &&
				alias !== null &&
				(verdict as OwnerVerdict).generation === identity.generation &&
				(verdict as OwnerVerdict).session_id === identity.sessionId &&
				(verdict as OwnerVerdict).server_key === identity.socketKey &&
				(verdict as OwnerVerdict).signal === "SIGTERM" &&
				(verdict as OwnerVerdict).result === "owner_term_then_session_cleanup" &&
				(verdict as OwnerVerdict).classification === "expected_operator_shutdown" &&
				(alias as OwnerVerdict & { owner_generation?: string }).owner_generation === identity.generation &&
				(alias as OwnerVerdict).session_id === identity.sessionId &&
				(alias as OwnerVerdict).server_key === identity.socketKey &&
				(alias as OwnerVerdict).signal === "SIGTERM" &&
				(alias as OwnerVerdict).result === "owner_term_then_session_cleanup" &&
				(alias as OwnerVerdict).classification === "expected_operator_shutdown" &&
				(alias as OwnerVerdict).intent_id === (verdict as OwnerVerdict).intent_id
			)
				return verdict as OwnerVerdict;
		} catch {}
	...
	await closeExactTmuxOwner(
		...
		},
		{
			...
			cleanupSession: async () => {
				...
				(deps.cleanupSession ?? ((name, targetEnv) => cleanupExactTmuxSession(name, targetEnv)))(
					nativeSessionId,
					env,
				);
			},
