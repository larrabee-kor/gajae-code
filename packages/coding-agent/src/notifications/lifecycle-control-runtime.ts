...
import {
	...
	buildGjcTmuxRequiredProfileCommands,
...
function lifecycleOwnerIsolationProbe(tmux: string, env: NodeJS.ProcessEnv): OwnerIsolationProbe {
	return {
		...
		probeServer: async (): Promise<TmuxServerProof> => {
			const result = Bun.spawnSync([tmux, "list-sessions", "-F", "#{pid}\t#{session_name}"], {
				stdout: "pipe",
				stderr: "pipe",
				env,
			});
...
function nativeTmuxSessionIdFromScopedReceipt(stdout: string): string | undefined {
	if (!isExactScopedBootstrapSuccessReceipt(stdout)) return undefined;
	return (JSON.parse(stdout.trim()) as { native_session_id: string }).native_session_id;
}

async function isLifecycleGenerationCurrent(stateDir: string, sessionId: string, generation: string): Promise<boolean> {
	try {
		const record: unknown = JSON.parse(
			await fsPromises.readFile(path.join(stateDir, sessionId, "owner-lifecycle", "generation.json"), "utf8"),
		);
		return (
			...
			(record as { schema_version?: unknown }).schema_version === 1 &&
			(record as { session_id?: unknown }).session_id === sessionId &&
			(record as { generation?: unknown }).generation === generation &&
...
async function executeLifecycleTmuxOwnerPlan(input: {
	...
	await replaceOwnerGeneration(input.stateDir, input.sessionId, input.generation);
	if (!(await isLifecycleGenerationCurrent(input.stateDir, input.sessionId, input.generation)))
	...
	const preSpawnProof =
		plan.execution.mode === "direct" && plan.server_state === "safe" ? await probe.probeServer("default") : undefined;
	if (
		preSpawnProof &&
	...
	if (created.exitCode !== 0) throw new Error("gjc_lifecycle_spawn_failed");
	const attempt: LifecycleAttemptExecution = {
		attemptCreated: true,
		nativeSessionId:
			plan.execution.mode === "scoped"
				? nativeTmuxSessionIdFromScopedReceipt(created.stdout.toString())
				: nativeTmuxSessionIdFromSpawn(created.stdout.toString()),
	};
	input.onAttemptCreated?.(attempt);
	if (!(await isLifecycleGenerationCurrent(input.stateDir, input.sessionId, input.generation)))
		throw new Error("gjc_lifecycle_owner_generation_changed");
	if (!attempt.nativeSessionId) throw new Error("gjc_lifecycle_spawn_failed");
	const proof = await probe.probeServer("default");
	if (
		proof.state !== "safe" ||
		!proof.pid ||
		!proof.startTime ||
...
async function cleanupLifecycleAttempt(input: {
	...
	if (!input.nativeSessionId || input.expectedServerPid === undefined || !input.expectedServerStartTime) return;
	try {
		const proof = await (
			input.ownerIsolationProbe ?? lifecycleOwnerIsolationProbe(input.tmux, input.env)
		).probeServer("default");
		if (
			proof.state !== "safe" ||
			!proof.pid ||
			...
			return;
		// Compensation is authorized only by the immutable native session id
		// captured from this exact successful spawn attempt.
		const target = input.nativeSessionId;
		Bun.spawnSync([input.tmux, "kill-session", "-t", target], {
			stdout: "ignore",
			stderr: "ignore",
			env: input.env,
		});
	} catch {
		// Compensation is best-effort and must not replace the originating failure.
	}
}
...
export function daemonSpawnCreate(
	...
	return async (
		...
		try {
			...
			if (!(await isLifecycleGenerationCurrent(stateDir, ids.intendedSessionId, generation)))
				throw new Error("gjc_lifecycle_owner_generation_changed");
			if (!ownerExecution.nativeSessionId) throw new Error("gjc_lifecycle_spawn_failed");
			const target = `${ownerExecution.nativeSessionId}:`;
			await applyRequiredLifecycleTmuxMetadata(tmux, target, env, {
				sessionId: ids.intendedSessionId,
				sessionStateFile,
				project: cwd,
				ownerGeneration: generation,
				ownerServerKey: "default",
			});
		} catch (error) {
...
async function applyRequiredLifecycleTmuxMetadata(
	...
	const requiredCommands = buildGjcTmuxRequiredProfileCommands(target, metadata);
	for (const command of requiredCommands) {
		const result = Bun.spawnSync([tmux, ...command.args], { stdout: "pipe", stderr: "pipe", env });
		if (result.exitCode !== 0) throw new Error("gjc_lifecycle_metadata_write_failed");
	}
	const requiredArgv = new Set(requiredCommands.map(command => JSON.stringify(command.args)));
	const profileCommands = buildGjcTmuxProfileCommands(target, env, metadata);
	for (const command of profileCommands) {
		if (requiredArgv.has(JSON.stringify(command.args))) continue;
		const optional = Bun.spawnSync([tmux, ...command.args], { stdout: "pipe", stderr: "pipe", env });
		if (optional.exitCode !== 0) {
			try {
				process.stderr.write("gjc lifecycle optional tmux profile command failed\n");
			} catch {
				// Optional profile diagnostics cannot affect lifecycle creation.
			}
		}
	}
}
...
export function daemonResumeSession(
	...
	return async (target: {
		...
		try {
			...
			if (!(await isLifecycleGenerationCurrent(stateDir, resumeId, generation)))
				throw new Error("gjc_lifecycle_owner_generation_changed");
			if (!ownerExecution.nativeSessionId) throw new Error("gjc_lifecycle_spawn_failed");
			const tgt = `${ownerExecution.nativeSessionId}:`;
			await applyRequiredLifecycleTmuxMetadata(tmux, tgt, env, {
				sessionId: resumeId,
				sessionStateFile,
