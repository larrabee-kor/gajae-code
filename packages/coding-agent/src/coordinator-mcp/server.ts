...
function isValidRuntimeSessionState(value: unknown, sessionId: string): value is RuntimeSessionStatePayload {
	...
	if (record.source === "process_postmortem") {
		...
		if (record.event === "owner_terminal") {
			const ownerTerminal = record.owner_terminal === undefined ? null : asRecord(record.owner_terminal);
			if (
				record.owner_terminal !== undefined &&
				(!ownerTerminal ||
					typeof ownerTerminal.generation !== "string" ||
					typeof ownerTerminal.socket_key !== "string" ||
					typeof ownerTerminal.classification !== "string" ||
					typeof ownerTerminal.observer !== "string" ||
					typeof ownerTerminal.observed_at !== "string" ||
					!Number.isFinite(Date.parse(ownerTerminal.observed_at)))
			)
				return false;
...
function isValidActiveTurnRecord(
	value: unknown,
	sessionId: string,
): value is { session_id: string; turn_id: string; status: TurnStatus; updated_at: string } {
...
async function coordinatorOwnerIsolationProbe(runner: CommandRunner): Promise<OwnerIsolationProbe> {
	return {
		...
		probeServer: async (socketKey): Promise<TmuxServerProof> => {
			...
			const startTime = ownerProcessStartTime(process.platform, stat);
			if (!startTime) return { state: "unverifiable", pid, sessionNames };
			if (process.platform !== "linux")
				return { state: "safe", pid, startTime, sessionNames, cgroup: { classification: "not_applicable" } };
...
async function startTmuxSession(
	...
	const ownerGeneration = await replaceOwnerGeneration(ownerStateDir, sessionName);
	...
	if (started.exitCode !== 0) throw new Error("coordinator_tmux_start_failed");
	const bootstrapReceiptValid =
		plan.execution.mode !== "scoped" || isExactScopedBootstrapSuccessReceipt(started.stdout);
	const initialProof = await proveCoordinatorOwnerServer(probe, socketKey);
	if (
		initialProof.state !== "safe" ||
		!initialProof.pid ||
		!initialProof.startTime ||
		(process.platform === "linux" && initialProof.cgroup?.classification !== "safe") ||
		(preSpawnProof && (initialProof.pid !== preSpawnProof.pid || initialProof.startTime !== preSpawnProof.startTime))
	)
		throw new Error("coordinator_tmux_owner_server_unverifiable");
	const cleanupFailedAttempt = async (): Promise<void> => {
		// Only scoped plans prove the socket was absent before this attempt.
		if (plan.execution.mode !== "scoped") return;
		const proof = await probe.probeServer(socketKey);
		if (
			proof.state === "safe" &&
			proof.pid === initialProof.pid &&
			proof.startTime === initialProof.startTime &&
			proof.sessionNames?.includes(plan.execution.attempt_session)
		)
			await runner(["tmux", "-L", socketKey, "kill-session", "-t", `=${plan.execution.attempt_session}`]);
	};
	if (!bootstrapReceiptValid) {
		await cleanupFailedAttempt();
		throw new Error("coordinator_tmux_start_failed");
	}
	...
	if (
		...
		await cleanupFailedAttempt();
		throw new Error("coordinator_tmux_owner_server_race");
	}
	const [tmuxTarget, paneId, nativeSessionId] = startRecord.split(/\s+/, 3);
	if (
		!tmuxTarget ||
		!paneId ||
		!nativeSessionId ||
		!tmuxTarget.startsWith(`${plan.execution.attempt_session}:`) ||
...
export function createCoordinatorMcpServer(options: CoordinatorMcpServerOptions = {}) {
	...
	async function reconcileActiveTurnAcknowledgements(): Promise<void> {
		await preflightCoordinatorMutation(namespaceDir, undefined, { rejectTerminalRuntimeEvidence: false });
	...
	async function callToolUnlocked(name: string, args: Record<string, unknown> = {}): Promise<Record<string, unknown>> {
		try {
			...
			return { ok: false, reason: publicCoordinatorError(error) };
		}
	}
