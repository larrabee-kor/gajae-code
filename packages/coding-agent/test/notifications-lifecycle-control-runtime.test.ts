...
describe("lifecycle control runtime", () => {
	...
	it("daemonResumeSession cold-restarts saved sessions from their recorded cwd", async () => {
		...
		expect(calls).toContain("@gjc-owner-server-key default");
	...
	it("daemon create propagates one generation into canonical lifecycle state and the resident child", async () => {
		...
		expect(calls).toContain("GJC_TMUX_OWNER_SERVER_KEY='default'");
		expect(calls).toContain(`@gjc-owner-generation ${generation}`);
		expect(calls).toContain("@gjc-owner-server-key default");
		expect(calls).not.toContain("GJC_OWNER_");
	...
	it("preserves a stale lifecycle spawn without metadata or unqualified compensation", async () => {
		...
		try {
			...
			expect(calls.some(call => call.startsWith("set-option ") || call.startsWith("kill-session "))).toBe(false);
	...
	it("stops required metadata writes at every failed write and cleans up only the attempted session", async () => {
		...
		fs.writeFileSync(
			...
			[
				...
				'printf "%s\\n" "$*" >> "$TMUX_CALLS"',
				'if [ "$1" = "new-session" ]; then printf \'$42\\n\'; fi',
				'if [ "$1" = "set-option" ]; then N=0; [ ! -f "$TMUX_COUNTER" ] || read -r N < "$TMUX_COUNTER"; N=$((N + 1)); printf "%s" "$N" > "$TMUX_COUNTER"; [ "$N" -eq "$TMUX_FAIL_INDEX" ] && exit 1; fi',
				"exit 0",
				"",
			].join("\n"),
		);
		...
		try {
			for (let failedWrite = 1; failedWrite <= 6; failedWrite++) {
				const counter = path.join(root, `counter-${failedWrite}`);
					...
					daemonSpawnCreate(
						{
							...
							GJC_TMUX_COMMAND: tmux,
							TMUX_CALLS: callsFile,
							TMUX_COUNTER: counter,
							TMUX_FAIL_INDEX: String(failedWrite),
						},
						{
							ownerIsolationProbe: {
								readCallerCgroup: async () => "/gjc-lifecycle-test.scope\n",
								probeServer: async () => {
									...
									return probeCalls === 1
										...
										: {
												...
												sessionNames: [`gjc_lc_metadata-${failedWrite}`],
											};
								},
							},
						},
					)(createFrame({ target: { kind: "existing_path", path: project } }), {
						lifecycleRequestId: `metadata-${failedWrite}`,
						intendedSessionId: `metadata-${failedWrite}`,
					}),
				).rejects.toThrow("gjc_lifecycle_metadata_write_failed");
				const calls = fs.readFileSync(callsFile, "utf8").trim().split("\n");
				const writes = calls.filter(call => call.startsWith("set-option "));
				expect(writes).toHaveLength(failedWrite);
				expect(calls.filter(call => call.startsWith("new-session "))).toHaveLength(1);
				expect(calls.filter(call => call === "kill-session -t $42")).toHaveLength(1);
			}
		} finally {
			fs.rmSync(root, { recursive: true, force: true });
		}
