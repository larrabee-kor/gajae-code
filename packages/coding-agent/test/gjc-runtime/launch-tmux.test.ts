...
import { isExactScopedBootstrapSuccessReceipt } from "@gajae-code/coding-agent/gjc-runtime/tmux-owner-isolation";
import {
...
function launchDefaultTmuxIfNeeded(context: TmuxLaunchContext): boolean {
	return launchDefaultTmuxIfNeededRaw(launchContext(context));
}
...
describe("default GJC tmux launch", () => {
	...
	it("preserves a new psmux session before resize or attach", () => {
	...
	it.each([
		...
		expect(calls.some(call => call[0] === "kill-session")).toBe(true);
...
it("pipes default control-command stderr while preserving interactive attach stderr", () => {
	...
	spyOn(Bun, "spawnSync").mockImplementation(options => {
		...
		if (command[1] === "new-session") return spawnResult(0, "$0");
...
it("preserves a newly created psmux session without name-only mutation, retry, attach, or cleanup", () => {
	const calls: Array<{ command: string; args: string[] }> = [];
	const diagnostics: string[] = [];
	const result = launchDefaultTmuxIfNeeded({
		parsed: args({ messages: ["hello world"], tmux: true }),
...
it("uses the captured native session ID for every post-create target", () => {
	...
	expect(targetFor("kill-session")).toEqual(["$0"]);
...
describe("tmux owner isolation launch gate", () => {
	...
	it("uses the scoped bootstrap receipt native ID for every post-create target", () => {
		...
		const handled = launchDefaultTmuxIfNeeded({
			...
			spawnSync: (command, spawnArgs) => {
				...
				if (command === "systemd-run")
					return {
						exitCode: 0,
						stdout: '{"schema_version":1,"ok":true,"code":"bootstrapped","native_session_id":"$42"}',
					};
				if (spawnArgs[0] === "attach-session") return { exitCode: 1, stderr: "attach failed" };
		...
		expect(targets("kill-session")).toEqual(["$42"]);
	});
	...
	it("preserves a replacement server when native create proof changes before profile mutation", () => {
		...
		expect(() => createGjcTmuxSession({ GJC_TMUX_COMMAND: "tmux", GJC_TMUX_SESSION: "managed" })).toThrow(
			"gjc_tmux_owner_changed_after_create",
		);
	...
	it("does not title-mutate a reusable psmux existing session", () => {
		const calls: string[][] = [];
		__setBinaryResolverForTests(candidate => (candidate === "psmux" ? "/fake/psmux" : null));
		try {
			const handled = launchDefaultTmuxIfNeeded({
			...
			expect(handled).toBe(true);
			expect(calls.some(call => call[0] === "set-option" || call[0] === "set-window-option")).toBe(false);
			expect(calls.filter(call => call[0] === "attach-session")).toHaveLength(1);
		} finally {
			__setBinaryResolverForTests(null);
		}
	...
	it("accepts only a bounded single-line receipt carrying an immutable native ID", () => {
		expect(
			isExactScopedBootstrapSuccessReceipt(
				'{"schema_version":1,"ok":true,"code":"bootstrapped","native_session_id":"$42"}',
			),
		).toBe(true);
	});
});
