...
try:
    ...
    identity = (set(verdict) == required and verdict["schema_version"] == 1 and verdict["generation"] == generation
        ...
        and current == {"schema_version": 1, "session_id": session, "generation": generation}
        and isinstance(verdict["observed_at"], str) and datetime.datetime.fromisoformat(verdict["observed_at"].replace("Z", "+00:00"))
    ...
    if verdict["classification"] == "expected_operator_shutdown":
        ...
        observed = datetime.datetime.fromisoformat(verdict["observed_at"].replace("Z", "+00:00"))
