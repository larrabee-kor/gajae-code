...
reconcile_prior_generation() {
  ...
  prior_generation="$(python3 - "$GENERATION_JSON" "$SESSION" <<'PY'
import json, sys
try:
    with open(sys.argv[1], encoding="utf-8") as handle: record = json.load(handle)
    value = record.get("generation")
    if record.get("schema_version") == 1 and record.get("session_id") == sys.argv[2] and isinstance(value, str) and value: print(value)
...
try:
    with open(sys.argv[1], encoding="utf-8") as handle: record = json.load(handle)
    valid = (set(record) == {"schema_version", "session_id", "generation"}
        and record["schema_version"] == 1
        and record["session_id"] == sys.argv[2]
        and isinstance(record["generation"], str) and bool(record["generation"]))
...
rollback_attempt_session() {
  ...
  if "$TMUX_BIN" -L "$SOCKET_KEY" has-session -t "=$target" >/dev/null 2>&1; then
    if ! "$TMUX_BIN" -L "$SOCKET_KEY" kill-session -t "=$target" >/dev/null 2>&1; then
      ROLLBACK_FAILURES="${ROLLBACK_FAILURES:+$ROLLBACK_FAILURES,}${label}_kill_failed"
      return 1
    fi
    probe=0
  else
  ...
  if [[ "$probe" -eq 0 ]]; then
    if "$TMUX_BIN" -L "$SOCKET_KEY" has-session -t "=$target" >/dev/null 2>&1; then
      ROLLBACK_FAILURES="${ROLLBACK_FAILURES:+$ROLLBACK_FAILURES,}${label}_still_present"
    elif [[ $? -eq 1 ]]; then
      return 0
    else
      ROLLBACK_FAILURES="${ROLLBACK_FAILURES:+$ROLLBACK_FAILURES,}${label}_probe_indeterminate"
    fi
  else
    ROLLBACK_FAILURES="${ROLLBACK_FAILURES:+$ROLLBACK_FAILURES,}${label}_probe_indeterminate"
  fi
...
rollback_attempt_sessions() {
  ...
  local failed=0
  rollback_attempt_session owner_session "$SESSION" || failed=1
  if [[ "$ROLLBACK_MONITOR_CREATED" == 1 ]]; then
    rollback_attempt_session monitor_session "$MONITOR_SESSION" || failed=1
  fi
  return "$failed"
}
...
temporary = f"{path}.{os.getpid()}.tmp"
with open(temporary, "x", encoding="utf-8") as handle:
    json.dump({"schema_version": 1, "session_id": session, "generation": generation}, handle, separators=(",", ":"))
    handle.write("\n")
...
PLAN_LINE="$(python3 - "$SESSION" "$OWNER_GENERATION" "$WORKDIR" "$STATE_DIR" "$SOCKET_KEY" "${TMUX_ARGV[@]}" <<'PY'
import json, sys
session, generation, cwd, state_dir, socket_key, *argv = sys.argv[1:]
print(json.dumps({"schema_version": 1, "op": "plan", "platform": "linux", "session_id": session, "owner_generation": generation, "cwd": cwd, "state_dir": state_dir, "socket_key": socket_key, "tmux_argv": argv}, separators=(",", ":")))
PY
)"
PLAN_RESPONSE="$(printf '%s\n' "$PLAN_LINE" | "$GJC_BIN" --internal-tmux-owner-isolation)" || { echo "owner-isolation plan protocol failed" >&2; exit 1; }
PLAN_MODE="$(python3 - "$PLAN_RESPONSE" "$SESSION" "$OWNER_GENERATION" "$STATE_DIR" "$SOCKET_KEY" "${TMUX_ARGV[@]}" <<'PY'
import datetime, json, sys
try:
    response = json.loads(sys.argv[1]); session, generation, state_dir, socket_key, argv = sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6:]; execution = response["execution"]
    ...
    if execution.get("mode") == "direct":
        valid = common and response.get("code") == "not_required" and response.get("server_state") in {"safe", "absent"} and classification in {"safe", "not_applicable"} and execution.get("argv") == argv
        ...
        valid = (
            ...
            and isinstance(scoped_argv, list) and scoped_argv[:6] == ["systemd-run", "--user", "--scope", "--quiet", "--unit", expected_scope] and len(scoped_argv) >= 7
            and attempt == {"token": token, "session_name": session, "socket_key": socket_key, "server_absent_before": True, "expires_at": expires_at}
...
python3 - "$POST_SPAWN_RESPONSE" <<'PY' || { echo "owner-isolation post-spawn server proof rejected" >&2; show_recovery_hint; exit 1; }
import json
import sys
try:
    response = json.loads(sys.argv[1])
...
for option_value in \
  ...
  option="${option_value%%=*}"
  value="${option_value#*=}"
  "$TMUX_BIN" -L "$SOCKET_KEY" set-option -t "=$SESSION:" "$option" "$value" >/dev/null 2>&1 || {
