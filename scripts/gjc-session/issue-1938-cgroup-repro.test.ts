...
describe("issue #1938 evidence", () => {
	...
	test("uses only private disposable resources for actual restart and counterfactual", () => {
		...
		expect(script).toContain('TRACKED_SERVERS=("${kept[@]}" "$tmpdir|$socket|$native_id|$pid|$start|$cgroup")');
		expect(script).toContain('tmux -L "$socket" kill-session -t "$native_id"');
	...
	test("accepts same-millisecond publication only when the captured identity changed and rejects malformed state", async () => {
		...
		try {
			...
			const baseline = await captureCanonicalVerdictBaseline(stateDir, sessionId);
			await Bun.write(verdictPath, JSON.stringify({ ...verdict, result: "owner_term_then_session_cleanup_v2" })); await Bun.write(path.join(stateDir, "verdict.json"), JSON.stringify({ ...verdict, result: "owner_term_then_session_cleanup_v2", owner_generation: generation })); await fs.utimes(verdictPath, trigger / 1000, trigger / 1000);
...
test("leaves no partial receipt when the Python serializer fails", async () => {
	...
	test("fails cleanup on indeterminate private tmux and systemd probes", async () => {
