...
cleanup() {
  ...
  for server in "${TRACKED_SERVERS[@]}"; do IFS='|' read -r tmpdir socket native_id pid start cgroup <<<"$server"; kill_owned_server "$tmpdir" "$socket" "$native_id" "$pid" "$start" "$cgroup" || cleanup_incomplete private_server; done
...
run_post_code() {
  ...
  if [[ -n "$raw_pid" && -n "$raw_cgroup" ]] && independent_scope "$raw_cgroup" && track_server "$WORKTREE/tmux" "$RAW_SOCKET" "$raw_session" "$raw_pid"; then owner_unit="$(owner_scope "$raw_cgroup")"; TRACKED_UNITS+=("$owner_unit"); append_case raw_proof_before_exec passed "$started" "$completed" "$raw_pid" "$raw_session" "$raw_cgroup" '' proven '' proven "$owner_unit"; else append_case raw_proof_before_exec failed "$started" "$completed" "$raw_pid" "$raw_session" "$raw_cgroup" '' proven '' unavailable "$SERVICE_UNIT"; return 1; fi
  ...
  owner_unit="$(owner_scope "$cgroup")"; TRACKED_UNITS+=("$owner_unit"); track_server "$WORKTREE/tmux" "$RECOVERY_SOCKET" "$recovery_session" "$pid" || { append_case unexpected_incident_recovery failed "$started" "$(now)" "$pid" "$recovery_session" "$cgroup" SIGTERM unexpected_owner_loss UNKNOWN unavailable; return 1; }
  ...
  pid="$(wait_server_pid "$recovery_session" "$RECOVERY_SOCKET" || true)"; cgroup="$(proc_cgroup "$pid")"; if [[ -z "$pid" || -z "$cgroup" ]] || ! independent_scope "$cgroup"; then append_case unexpected_incident_recovery failed "$started" "$(now)" "$pid" "$recovery_session" "$cgroup" SIGTERM unexpected_owner_loss UNKNOWN recovery_scope_unavailable; return 1; fi; owner_unit="$(owner_scope "$cgroup")"; TRACKED_UNITS+=("$owner_unit"); track_server "$WORKTREE/tmux" "$RECOVERY_SOCKET" "$recovery_session" "$pid" || { append_case unexpected_incident_recovery failed "$started" "$(now)" "$pid" "$recovery_session" "$cgroup" SIGTERM unexpected_owner_loss UNKNOWN recovery_identity_unavailable; return 1; }
