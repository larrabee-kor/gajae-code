...
mode = sys.argv[2]
if request["op"] == "plan":
    execution = {"mode": "direct", "argv": request["tmux_argv"], "attempt_session": request["session_id"]}
    seen = os.path.join(request["state_dir"], ".fixture-plan-seen")
    post_spawn = mode == "scoped" and os.path.exists(seen)
    if mode == "scoped" and not post_spawn:
        open(seen, "w", encoding="utf-8").close()

    if mode == "scoped" and not post_spawn:
        token = "123e4567-e89b-12d3-a456-426614174099"
        expected_scope = f"gjc-owner-{token}.scope"
        attempt = {"token":token,"session_name":request["session_id"],"socket_key":request["socket_key"],"server_absent_before":True,"expires_at":"2099-01-01T00:00:00.000Z"}
        ...
        execution = {
            "mode": "scoped",
            "argv": ["systemd-run", "--user", "--scope", "--quiet", "--unit", expected_scope, "bash", "-c", """read -r bootstrap; [ "$bootstrap" = "$1" ] || exit 9; shift; native_session_id="$("$@")" || exit $?; printf '{"schema_version":1,"ok":true,"code":"bootstrapped","native_session_id":"%s"}\\n' "$native_session_id" """, "bootstrap", stdin_line, *request["tmux_argv"]],
...
describe("gjc-session create public owner lifecycle", () => {
	...
	test("fails closed for malformed plans before any router registration", async () => {
		...
		await executable(bad, "#!/usr/bin/env bash\nif [[ $1 == --internal-tmux-owner-isolation ]]; then echo '{}'; fi\n");
...
if __import__("os").path.exists(__import__("os").environ["GJC_FIXTURE_POSTSPAWN"]): print("{}")
 ...
 print(json.dumps({"schema_version":1,"ok":True,"code":"not_required","execution":{"mode":"direct","argv":request["tmux_argv"],"attempt_session":request["session_id"]},"server_state":"safe","classification":{"classification":"safe"}}))
`);
	await fs.mkdir(state, { recursive: true });
	...
	expect(result.exitCode).toBe(1);
	const generation = ((await Bun.file(path.join(state, name, "owner-lifecycle", "generation.json")).json()) as { generation: string }).generation;
	const failed = await Bun.file(path.join(state, name, "owner-lifecycle", `creation-failed-${generation}.json`)).json() as Record<string, unknown>;
	expect(failed).toMatchObject({ kind: "creation_failed", session_id: name, owner_generation: generation, boundary: "postspawn" });
	expect(await Bun.file(path.join(state, "creation-state.json")).json()).toEqual(failed);
	expect(await Bun.file(path.join(state, "metadata.json")).json()).toMatchObject({ owner_generation: generation });
...
test("retries a generation-bound failed create without synthesizing owner loss or recovery", async () => {
	...
	const lifecycle = path.join(state, name, "owner-lifecycle"); const failedGeneration = ((await Bun.file(path.join(lifecycle, "generation.json")).json()) as { generation: string }).generation;
	expect(await Bun.file(path.join(lifecycle, `creation-failed-${failedGeneration}.json`)).exists()).toBe(true);
	expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ GJC_BIN: bin, GJC_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
...
test("records rollback and failure-alias publication faults", async () => {
	for (const kind of ["rollback", "probe", "alias", "canonical-collision"] as const) {
		...
		await executable(tmux, `#!/usr/bin/env bash
if [[ "$*" == *"set-option"* ]]; then
  if [[ "${"$"}{GJC_FIXTURE_CLEANUP_KIND}" == alias ]]; then rm -f "${"$"}{GJC_FIXTURE_STATE}/creation-state.json"; mkdir -p "${"$"}{GJC_FIXTURE_STATE}/creation-state.json"; fi
...
if [[ "${"$"}{GJC_FIXTURE_CLEANUP_KIND}" == probe && -e "${"$"}{GJC_FIXTURE_PROBE_MARKER}" && "$*" == *"has-session"* ]]; then exit 25; fi
exec tmux "$@"
`);
		...
		if (kind === "canonical-collision") {
			...
			const canonical = { schema_version: 1, kind: "creation_cleanup_failed", session_id: name, owner_generation: generation, exit_code: 99, rollback_failures: ["owner_session"], failure_publication_failed: false };
			const alias = { ...canonical, exit_code: 98 };
			expect(await Bun.file(path.join(lifecycle, `creation-cleanup-failure-${generation}.json`)).json()).toEqual(canonical);
			expect(await Bun.file(path.join(state, "creation-cleanup-failure.json")).json()).toEqual(alias);
		} else {
			const failure = await Bun.file(path.join(state, "creation-cleanup-failure.json")).json() as Record<string, unknown>;
			if (kind === "rollback") expect(failure).toMatchObject({ kind: "creation_cleanup_failed", rollback_failures: ["owner_session_kill_failed"], failure_publication_failed: false });
			else if (kind === "probe") expect(failure).toMatchObject({ kind: "creation_cleanup_failed", rollback_failures: ["owner_session_probe_indeterminate"], failure_publication_failed: false });
			else expect(failure).toMatchObject({ kind: "creation_cleanup_failed", rollback_failures: [], failure_publication_failed: true });
		}
	}
});
...
`);
	...
	const aliases: Record<string, Record<string, unknown>> = {
		"metadata.json": await Bun.file(path.join(state, "metadata.json")).json() as Record<string, unknown>,
		"creation-state.json": await Bun.file(path.join(state, "creation-state.json")).json() as Record<string, unknown>,
		"started.json": { schema_version: 1, kind: "started", session_id: name, owner_generation: replacementGeneration },
		"prompt-accepted.json": { schema_version: 1, kind: "prompt_accepted", session_id: name, owner_generation: replacementGeneration },
		"terminal.json": { schema_version: 1, kind: "terminal", session_id: name, owner_generation: replacementGeneration },
		"final.json": { schema_version: 1, kind: "terminal", session_id: name, owner_generation: replacementGeneration },
	};
	for (const [alias, record] of Object.entries(aliases)) await Bun.write(path.join(state, alias), JSON.stringify(record));
	for (const [alias, record] of Object.entries(aliases)) {
		const canonical = { ...record, owner_generation: oldGeneration };
		await Bun.write(path.join(lifecycle, `${alias.replace(".json", "")}-${oldGeneration}.json`), JSON.stringify(canonical));
	}
	const publisherStarted = path.join(root, "publisher-started"); const publisherDone = path.join(root, "publisher-done");
	const publisher = Bun.spawn(["bash", "-c", 'source "$1"; lifecycle="$2"; state="$3"; generation_path="$4"; session="$5"; generation="$6"; started="$7"; done_marker="$8"; touch "$started"; status=0; for pair in "metadata metadata.json" "creation-state creation-state.json" "started started.json" "prompt-accepted prompt-accepted.json" "terminal terminal.json" "final final.json"; do set -- $pair; gjc_session_publish_current_alias "$lifecycle/$1-$generation.json" "$state/$2" "$generation_path" "$session" "$generation" || status=1; done; touch "$done_marker"; exit "$status"', "publisher", postmortemScript, lifecycle, state, path.join(lifecycle, "generation.json"), name, oldGeneration, publisherStarted, publisherDone], { stdout: "pipe", stderr: "pipe" });
	...
	for (const [alias, expected] of Object.entries(aliases)) expect(await Bun.file(path.join(state, alias)).json()).toEqual(expected);
}, 20_000);

test("treats a prior generation without an exact started receipt as interrupted creation", async () => {
...
test("fails closed and byte-preserves malformed, corrupt, or mismatched current generation state", async () => {
	for (const [label, body] of [
		...
		const lifecycle = path.join(state, name, "owner-lifecycle"); await fs.mkdir(lifecycle, { recursive: true });
		const bytes = typeof body === "string" && (label === "schema" || label === "empty") ? body.replace("placeholder", name) : body;
